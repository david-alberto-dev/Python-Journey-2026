...
while True:
    print("""
    --- Shape Math Tool ---
    Type '2D' to calculate the area of a 2D shape
    Type '3D' to calculate the volume of a 3D shape
    Type 'exit' to exit
    """)
    ...
    match option:
        ...
        case "2d":
            print(f"Available 2D shapes: {', '.join(calc.shapes_2d.keys())}")

            choice = input("Enter shape name: ").lower().strip()

            if choice in calc.shapes_2d:
                requirements = calc.shape_inputs[choice]
                values = []
                for item in requirements:
                    num = calc.get_number(f"Enter {item}: ")
                    values.append(num)
                result = calc.shapes_2d[choice](*values)
                print(f"Result: {result:.2f} square units")
            else:
                print("Shape not found!")

        case "3d":
            print(f"Available 3D shapes: {', '.join(calc.shapes_3d.keys())}")

            choice = input("Enter shape name: ").lower().strip()

            if choice in calc.shapes_3d:
                requirements = calc.shape_inputs[choice]
                values = []
                for item in requirements:
                    num = calc.get_number(f"Enter {item}: ")
                    values.append(num)
                result = calc.shapes_3d[choice](*values)
